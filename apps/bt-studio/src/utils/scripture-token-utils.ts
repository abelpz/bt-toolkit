...
export function extractTokensFromVerseRange(
  ...
  console.log(`🔍 Extracting tokens from ${reference.book} ${startChapter}:${startVerse} to ${endChapter}:${endVerse}`);
  ...
  for (const chapter of scripture.chapters) {
    ...
    for (const verse of chapter.verses) {
      ...
      if (isInRange) {
        ...
        console.log(`📝 Added ${verse.tokens.length} tokens from ${chapter.number}:${verse.number}`);
      }
    }
  }

  console.log(`✅ Extracted ${tokens.length} total tokens from verse range`);
  return tokens;
}
