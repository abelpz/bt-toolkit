...
/**
 * Load a passage set from a JSON file (Node.js/file system)
 */
export async function loadPassageSetFromFile(filePath: string): Promise<PassageSet> {
  try {
    ...
    throw new Error(`Failed to load passage set from file: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}

/**
 * Save a passage set to a JSON file (Node.js/file system)
 */
export async function savePassageSetToFile(passageSet: PassageSet, filePath: string): Promise<void> {
  try {
    const fs = await import('fs/promises');
    const jsonContent = serializePassageSet(passageSet);
    await fs.writeFile(filePath, jsonContent, 'utf-8');
  } catch (error) {
    throw new Error(`Failed to save passage set to file: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
