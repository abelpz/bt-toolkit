...
export class QuestionsProcessor {
  ...
  async processQuestions(
    ...
    console.log(`📝 Processing Translation Questions for ${bookCode} (${bookName})`);
    ...
    try {
      ...
      console.log(`✅ Questions processing completed in ${processingTime}ms`);
      console.log(`📊 Questions summary: ${questions.length} total, ${chaptersWithQuestions.length} chapters`);
  ...
  private parseTsvContent(tsvContent: string): TranslationQuestion[] {
    ...
    console.log(`📋 TSV header columns:`, header);
    ...
    header.forEach((column, index) => {
      ...
      switch (normalizedColumn) {
        ...
        case 'response':
          ...
          break;
        default:
          console.log(`🔍 Unknown column: ${column} at index ${index}`);
      }
    });
