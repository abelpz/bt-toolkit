...
import { ResourceManager, ResourceAdapter, ResourceMetadata } from '../../types/context'
...
export class ResourceConfigProcessor implements IResourceConfigProcessor {
  async process(
    ...
    console.log(`🔄 Processing ${appResourceConfigs.length} resource configurations...`)
    ...
    console.log('🔧 Creating resource adapters...')
    for (const config of sortedConfigs) {
      try {
        console.log(`📦 Creating adapter for: ${config.panelResourceId}`)
        console.log(`📋 Adapter config:`, config.adapterConfig)
        ...
        console.log(`✅ Created adapter for: ${config.panelResourceId}`)
        console.log(`🔍 Adapter resourceId: ${adapter.resourceId}, resourcePriority:`, (adapter as any).resourcePriority)
      } catch (error) {
    ...
    console.log(`🔧 Initializing ResourceManager with ${adapters.length} adapters...`)
    if (!storageAdapter) {
    ...
    console.log('📋 Loading resource metadata...')
    ...
    for (let i = 0; i < sortedConfigs.length && i < adapters.length; i++) {
      ...
      try {
        ...
        console.log(`📋 Loading metadata for ${config.panelResourceId}: ${resourceServer}/${resourceOwner}/${resourceLanguage}`)
        
        // Load metadata through ResourceManager's targeted caching method
        console.log(`🔍 Loading metadata via ResourceManager for ${config.panelResourceId}`)
        ...
        console.log(`✅ Loaded metadata for ${config.panelResourceId}: ${matchingMetadata.title}`)
        ...
        console.log(`✅ Processed resource: ${config.panelResourceId} -> ${matchingMetadata.title}`)
      } catch (error) {
    ...
    console.log(`✅ Resource processing complete: ${processedConfigs.length} resources, anchor: ${anchorResource.panelResourceId}`)
  ...
  private generatePanelConfig(processedConfigs: ProcessedAppResourceConfig[]): LinkedPanelsConfig {
    ...
    console.log('🔧 Generated panel config from PANEL_ASSIGNMENTS:', { 
