...
export class IndexedDBStorageAdapter implements StorageAdapter {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(`📦 Initializing IndexedDB storage: ${this.dbName}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        console.log(`🔄 Upgrading IndexedDB schema to version ${this.dbVersion}`);
  ...
  private createObjectStores(db: IDBDatabase): void {
    ...
    if (!db.objectStoreNames.contains('resource_metadata')) {
      ...
      console.log(`📋 Created resource_metadata object store`);
    }
    ...
    if (!db.objectStoreNames.contains('resource_content')) {
      ...
      console.log(`📋 Created resource_content object store`);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        if (metadata.length > 0) {
          console.log(`🔍 First metadata record id: ${metadata[0].id}, resourceKey: ${metadata[0].resourceKey}`);
        }
        resolve(metadata);
      };
    });
  ...
  async saveResourceMetadata(metadata: ResourceMetadata[]): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const meta of metadata) {
        ...
        console.log(`💾 Saving metadata with id: ${meta.id}, resourceKey: ${resourceKey}`);
        const dataToStore = {
          ...meta,
      ...
      for (const key of keys) {
        ...
        request.onsuccess = () => {
          ...
          if (completed === keys.length) {
            console.log(`📊 Found ${results.length}/${keys.length} content items`);
            resolve(results);
          }
        };
  ...
  async clearExpiredContent(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.oncomplete = () => {
        console.log(`✅ Cleared ${deletedCount} expired content items`);
        resolve();
      };
  ...
  async clearAllContent(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.oncomplete = () => {
        console.log(`✅ Cleared all content items`);
        resolve();
      };
  ...
  async close(): Promise<void> {
    if (this.db) {
      console.log(`🔒 Closing IndexedDB database: ${this.dbName}`);
      this.db.close();
      this.db = null;
  ...
  async deleteDatabase(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      console.log(`🗑️ Deleting IndexedDB database: ${this.dbName}`);
