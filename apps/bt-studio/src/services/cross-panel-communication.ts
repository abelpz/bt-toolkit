...
import type { WordToken, ProcessedChapter, ProcessedVerse, OptimizedToken, OptimizedChapter, OptimizedVerse, OptimizedScripture } from './usfm-processor';
...
export class CrossPanelCommunicationService {
  ...
  removePanel(resourceId: string): void {
    ...
    this.activePanels.delete(resourceId);
    console.log(`🗑️ Completely removed panel: ${resourceId}`);
  }
  ...
  handleTokenClick(
    ...
    console.log('🖱️ Token clicked:', {
      tokenId: 'id' in clickedToken ? clickedToken.id : clickedToken.uniqueId,
      content: 'text' in clickedToken ? clickedToken.text : clickedToken.content,
      sourceResourceId
    });
  ...
  private createUnifiedOriginalLanguageTokenOptimized(
    ...
    if (sourcePanel?.isOriginalLanguage) {
      ...
      console.log('🎯 Original language click (optimized) - using clicked token directly');
      ...
      console.log('🎯 Target language click (optimized) - using align array');
  ...
  private createUnifiedOriginalLanguageToken(
    ...
    if (sourcePanel?.isOriginalLanguage) {
      // Original language panel: clicked token IS the original language token
      console.log('🎯 Original language click - using clicked token directly');
      ...
      if (!strong) {
        ...
        console.log('⚠️ No alignment data found, using fallback strong:', strong);
      }
      
      console.log('🔍 Original language token details:', {
        hasAlignment: !!clickedToken.alignment,
        strong: strong,
        lemma: lemma,
        morph: morph,
        content: clickedToken.content,
        uniqueId: clickedToken.uniqueId
      });
        ...
        if (panelData.resourceType === 'ULT' || panelData.resourceType === 'UST') {
          ...
          for (const chapter of panelData.chapters) {
            for (const verse of chapter.verses) {
              ...
              // Debug: Log all alignment groups for this verse
              console.log('🔍 All alignment groups in verse:', verse.alignmentGroups.map(g => ({
                id: g.id,
                sourceWords: g.sourceWords,
                targetTokenCount: g.targetTokens?.length || 0
              })));
              ...
              for (const group of verse.alignmentGroups) {
                if (group.sourceWords.includes(clickedToken.content)) {
                  ...
                  console.log('🎯 Found alignment group for original language word:', {
                    word: clickedToken.content,
                    groupId: alignmentGroupId,
                    sourceWords: group.sourceWords,
                    targetTokens: group.targetTokens?.map(t => t.content) || []
                  });
                  break;
                }
              }
      ...
      console.log('🎯 Target language click - using alignedOriginalWordIds');
  ...
  private createOriginalLanguageTokenFromGroup(
    ...
    for (const chapter of sourcePanel.chapters) {
      for (const verse of chapter.verses) {
        ...
        if (alignmentGroup) {
          console.log('🎯 Found alignment group:', alignmentGroup);
  ...
  async handleWordClick(
    ...
    console.log(`🖱️ Word clicked (legacy): "${clickedToken.content}" in ${sourceResourceId}`);
