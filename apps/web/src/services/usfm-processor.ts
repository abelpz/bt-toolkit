...
export class USFMProcessor {
  ...
  async processUSFM(
    ...
    try {
      console.log(`🔄 Processing USFM for ${bookCode} (${usfmContent.length} chars)...`);
      
      // Debug: Check raw USFM content for \ts\* markers
      const tsMarkerCount = (usfmContent.match(/\\ts\\\*/g) || []).length;
      console.log(`   Raw USFM contains ${tsMarkerCount} \\ts\\* markers`);
      
      if (tsMarkerCount > 0) {
        console.log(`   Sample \\ts\\* context:`);
        const tsMatches = usfmContent.match(/(.{0,50}\\ts\\\*.{0,50})/g);
        tsMatches?.slice(0, 3).forEach((match, i) => {
      ...
      if (translatorSections.length === 0) {
        console.log(`📚 No sections found in USFM for ${bookCode}, checking default sections...`);
        try {
          const defaultSections = await defaultSectionsService.getDefaultSections(bookCode);
          if (defaultSections.length > 0) {
            translatorSections = defaultSections;
            console.log(`✅ Using ${defaultSections.length} default sections for ${bookCode}`);
          }
        } catch (error) {
      ...
      console.log(`✅ USFM processing complete in ${processingDuration}ms`);
      console.log(`   Chapters: ${metadata.statistics.totalChapters}`);
      console.log(`   Verses: ${metadata.statistics.totalVerses}`);
      console.log(`   Paragraphs: ${metadata.statistics.totalParagraphs}`);
      console.log(`   Sections: ${metadata.statistics.totalSections}`);
      console.log(`   Alignments: ${metadata.statistics.totalAlignments}`);
  ...
  private processChapter(
    ...
    if (chapterData.front?.verseObjects) {
      ...
      if (frontParagraphObj) {
        ...
        console.log(`📝 Found initial paragraph marker in front: ${nextParagraphStyle}`);
      }
    }
    ...
    for (let i = 0; i < verseEntries.length; i++) {
      ...
      if (!verse.text.trim()) {
        console.log(`⚠️ Skipping empty verse ${bookCode} ${chapterNum}:${verseNumStr}`);
        continue;
      }
      ...
      if (shouldStartNewParagraph) {
        ...
        console.log(`📝 Started new paragraph ${currentParagraph.id} with style '${styleForThisParagraph}' at verse ${parsedVerse.number}`);
        ...
        console.log(`📝 Added verse ${parsedVerse.number} to paragraph ${currentParagraph.id}`);
      }
      ...
      if (hasParagraphMarker && paragraphStyle) {
        ...
        console.log(`📝 Verse ${parsedVerse.number} has paragraph marker '${paragraphStyle}' for next paragraph`);
      }
  ...
  private processVerse(
    ...
    // Log verse spans for debugging
    if (parsedVerse.isSpan) {
      console.log(`📏 Found verse span: ${parsedVerse.originalVerseString} (${parsedVerse.spanStart}-${parsedVerse.spanEnd}) in ${bookCode} ${chapterNum}`);
    }
  ...
  private extractTranslatorSections(usfmJson: USFMDocument, bookCode: string): TranslatorSection[] {
    ...
    console.log(`🔍 Searching for \\ts\\* markers in USFM JSON...`);
    ...
    console.log(`   Found ${tsMarkerCount} \\ts\\* markers and ${tsMarkerCountAlt} ts\\* markers in full JSON`);
    ...
    tsVariations.forEach((variation, index) => {
      ...
      console.log(`   Pattern ${index + 1} (${variation.name}): ${matches} matches`);
    });
    ...
    if (usfmJson.headers) {
      ...
      if (headersJson.includes('"tag":"ts"') || headersJson.includes('"tag":"ts\\\\*"')) {
        console.log(`   Found pre-chapter \\ts\\* marker in headers`);
        currentSection = {
          startChapter: 1,
          startVerse: 1,
          startVerseString: "1"
    ...
    for (const [chapterNumStr, chapterData] of Object.entries(usfmJson.chapters)) {
      ...
      for (const [verseNumStr, verseData] of Object.entries(chapterData)) {
        ...
        if (hasTranslationSection) {
          console.log(`   Found \\ts\\* marker at ${chapterNum}:${parsedVerse.originalVerseString}`);
    ...
    console.log(`✅ Extracted ${sections.length} translator sections`);
    sections.forEach((section, index) => {
      ...
      console.log(`   Section ${index + 1}: ${startRef} - ${endRef}`);
    });
