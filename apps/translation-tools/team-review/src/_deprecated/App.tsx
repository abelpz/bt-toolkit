...
import { SIGNAL_TYPES, WordClickedPayload } from './types/signaling';
...
function App() {
  ...
  const [topPanelResources, setTopPanelResources] = useState<PanelType[]>(['ult', 'ust', 'tn', 'alignment']);
  const [bottomPanelResources, setBottomPanelResources] = useState<PanelType[]>(['ult', 'ust', 'tn', 'alignment']);
  const [isSettingsOpen, setIsSettingsOpen] = useState(false);
  const [isCommentsOpen, setIsCommentsOpen] = useState(false);

  // Get SignalBus instance for signaling
  ...
  // Handle word clicks to show alignment info and open alignment panel in opposite panel
  const handleWordClick = async (word: AlignedWord, index: number, sourcePanelPosition?: 'top' | 'bottom') => {
    if (word.alignment) {
      console.log('Word clicked:', word, 'from panel:', sourcePanelPosition);
      
      // Determine the opposite panel from where the click originated
      let targetPanelId: string;
      let targetSetPanel: (panel: PanelType) => void;
      
      if (sourcePanelPosition === 'top') {
        // Click came from top panel, open alignment in bottom panel
        targetPanelId = 'bottom-panel';
        targetSetPanel = setBottomPanel;
      } else if (sourcePanelPosition === 'bottom') {
        // Click came from bottom panel, open alignment in top panel
        targetPanelId = 'top-panel';
        targetSetPanel = setTopPanel;
      } else {
        // Fallback: if we can't determine source, use the panel that's NOT currently showing alignment
        if (topPanel === 'alignment') {
          targetPanelId = 'bottom-panel';
          targetSetPanel = setBottomPanel;
        } else {
          targetPanelId = 'top-panel';
      ...
      // Switch the target panel to alignment
      targetSetPanel('alignment');
      
      // Small delay to ensure panel switch happens before signal
      setTimeout(async () => {
        // Emit word clicked signal to notify alignment resource
        await signalBus.emit<WordClickedPayload>({
          type: SIGNAL_TYPES.WORD_CLICKED,
          source: {
            panelId: sourcePanelPosition === 'top' ? 'top-panel' : 'bottom-panel',
            resourceId: 'verse-display'
          },
          target: {
            panelId: targetPanelId,
            resourceId: `alignment-resource-${targetPanelId === 'top-panel' ? 'top' : 'bottom'}`
          },
          payload: {
            word: word.text,
            ...
            wordIndex: index,
            verseRef: '1:1',
            sourceResourceId: 'verse-display',
