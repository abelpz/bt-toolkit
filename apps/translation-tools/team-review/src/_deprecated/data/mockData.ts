import { BookResources, VerseText, TranslationNote, TranslationWord, TranslationWordLink, ReviewComment, AlignedWord } from '../types';
...
const createRomans1_1_UST = (): VerseText => {
  ...
  return {
    reference: "1:1",
    words,
    rawText: "Yo soy Pablo. Sirvo a Cristo Jesús. Dios me escogió para ser apóstol"
  };
};
...
const mockTranslationWords: TranslationWord[] = [
  {
    id: "tw001",
    term: "siervo",
    definition: "Un siervo es una persona que trabaja para otra persona, ya sea por su propia voluntad o porque fue forzado a hacerlo.",
    examples: [
  ...
  },
  {
    id: "tw002",
    term: "apóstol",
    definition: "Un apóstol es alguien que es enviado con un mensaje especial. Los doce apóstoles fueron escogidos por Jesús para predicar el evangelio.",
    examples: [
...
const mockReviewComments: ReviewComment[] = [
  {
    ...
    comment: "¿Deberíamos usar 'esclavo' en lugar de 'siervo' para ser más literal al griego δοῦλος? Creo que 'esclavo' transmite mejor la idea de total sumisión que Pablo quiere expresar.",
  ...
  },
  {
    ...
    comment: "Esta explicación está muy clara. Me gusta cómo explica el concepto de 'llamado'. Es fácil de entender para lectores nuevos.",
    author: "Carlos Ruiz",
  ...
  },
  {
    ...
    comment: "Excelente explicación del contexto cultural. Esto ayudará mucho a los traductores a entender la profundidad del término.",
    author: "Ana López",
  ...
  },
  {
    ...
    comment: "La traducción 'llamado apóstol' es buena, pero ¿deberíamos considerar 'apóstol por llamamiento' para ser más claro?",
  ...
  },
  {
    ...
    comment: "Me parece que esta explicación simplifica bien el concepto sin perder el significado teológico importante.",
    author: "Elena Vargas",
  ...
  },
  {
    ...
    comment: "Sería útil explicar por qué Pablo usa 'Cristo Jesús' en lugar de 'Jesucristo' en esta carta. ¿Hay algún significado especial?",
...
export const getVerseByReference = (verses: VerseText[], reference: string): VerseText | undefined => {
  return verses.find(verse => verse.reference === reference);
};

// Helper function to get notes for a verse
export const getNotesForVerse = (notes: TranslationNote[], reference: string): TranslationNote[] => {
  return notes.filter(note => note.reference === reference);
};

// Helper function to get comments for a verse
export const getCommentsForVerse = (comments: ReviewComment[], reference: string): ReviewComment[] => {
