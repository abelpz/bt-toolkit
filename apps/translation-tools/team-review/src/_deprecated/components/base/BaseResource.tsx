...
export function useBaseResource(panelId: PanelId, resourceId: ResourceId) {
          ...
          switch (signal.type) {
            ...
            case 'hide_panel':
              if (signal.target?.panelId === panelId) {
                ...
                signalBus.emit({
                  type: 'resource_dismissed',
                  source: { panelId, resourceId },
                  payload: {
                    resourceId,
                    resourceType: 'unknown', // Can be overridden by specific resources
                    reason: 'hidden'
                  }
                });
              }
  ...
  }, [panelId, resourceId, signalBus]); // Create resource only once

  // Create stable emitSignal function
  ...
  return {
    state,
    setState,
    emitSignal: emitSignalRef.current,
