...
export const TranslationNotesCard: React.FC<TranslationNotesCardProps> = ({ notes, resourceId = 'translation-notes' }) => {
  ...
  const goToNext = () => {
    if (currentIndex < (notes?.length || 0) - 1) {
      setCurrentIndex(currentIndex + 1);
    }
  };
  ...
  useEffect(() => {
    if (notes && notes.length > 0 && notes[currentIndex]) {
      sendHighlightMessage(notes[currentIndex]);
    }
  }, [currentIndex, notes]);
  ...
  const canGoDown = currentIndex < notes.length - 1;
